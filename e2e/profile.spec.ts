...
test.describe("Perfil de Usuario", () => {
  ...
  test.describe("Visualización del perfil", () => {
    ...
    test("muestra estadísticas del perfil", async ({ page }) => {
      ...
      if (await statsSection.isVisible().catch(() => false)) {
        ...
        // Verificar IMC
        const bmi = await page
          .locator(SELECTORS.profile.bmiValue)
          .isVisible()
          .catch(() => false);

        // Verificar calorías diarias
        const calories = await page
          .locator(SELECTORS.profile.dailyCalories)
          .isVisible()
          .catch(() => false);
  ...
  test.describe("Edición del perfil", () => {
    test("puede abrir el modo de edición", async ({ page }) => {
      ...
      await expect(
        page.locator(SELECTORS.profile.firstNameInput).first(),
      ).toBeVisible();
    });
    ...
    test("puede actualizar el peso del usuario", async ({ page }) => {
      ...
      const saveButtonVisible = await page
        .locator(SELECTORS.profile.saveButton)
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Gestión de cuenta", () => {
    ...
    test("puede navegar a otras secciones desde el perfil", async ({
      page,
    }) => {
      // Volver a home
  ...
  test.describe("Recetas guardadas", () => {
    test("accede a recetas guardadas desde el perfil", async ({ page }) => {
      ...
      const savedRecipesLink = page
        .locator(
          'a:has-text("Recetas"), button:has-text("Recetas"), a:has-text("Guardadas")',
        )
        .first();
