...
test.describe("Navegación", () => {
  ...
  test.describe("Navegación principal (Bottom Tab Bar)", () => {
    ...
    test("mantiene el estado al navegar entre pestañas", async ({ page }) => {
      ...
      await expect(
        page.locator(SELECTORS.recommendation.recommendationResult).first(),
      ).toBeVisible();
    });
  });
  ...
  test.describe("URLs y rutas", () => {
    test("accede directamente a /recommendation estando autenticado", async ({
      page,
    }) => {
    ...
    test("accede directamente a /profile estando autenticado", async ({
      page,
    }) => {
    ...
    test("accede directamente a /pantry estando autenticado", async ({
      page,
    }) => {
    ...
    test("accede directamente a /plan estando autenticado", async ({
      page,
    }) => {
  ...
  test.describe("Página 404", () => {
    test("muestra página 404 para rutas inexistentes", async ({ page }) => {
      ...
      const notFoundVisible = await page
        .locator("text=404, No encontrado, Página no existe")
        .isVisible()
        .catch(() => false);
      // O redirigir a home
      const isHome =
