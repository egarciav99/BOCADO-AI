...
test.describe('Recomendaciones', () => {
  ...
  test.describe('Generación de recomendaciones', () => {
    test('puede generar recomendación "En casa" para desayuno', async ({ page }) => {
      ...
      await expect(page.locator(SELECTORS.recommendation.recipeTitle).first()).toBeVisible();
    });

    test('puede generar recomendación "En casa" para almuerzo', async ({ page }) => {
      await page.click(SELECTORS.recommendation.homeOption);
    ...
    test('puede generar recomendación "En casa" para cena', async ({ page }) => {
      await page.click(SELECTORS.recommendation.homeOption);
    ...
    test('puede generar recomendación "Fuera" para comida', async ({ page }) => {
    ...
    test('muestra información nutricional en la recomendación', async ({ page }) => {
      await page.click(SELECTORS.recommendation.homeOption);
      await page.click(SELECTORS.recommendation.mealTypeLunch);
      await page.click(SELECTORS.recommendation.generateButton);
      ...
      const hasCalories = await page.locator(SELECTORS.recommendation.calories).isVisible().catch(() => false);
      
      // O verificar que hay contenido de macros
      const hasMacros = await page.locator(SELECTORS.recommendation.macros).isVisible().catch(() => false);
  ...
  test.describe('Interacción con recetas', () => {
    ...
    test('puede guardar una receta', async ({ page }) => {
      const saveButton = page.locator(SELECTORS.recommendation.saveRecipeButton).first();
      ...
      if (await saveButton.isVisible().catch(() => false)) {
        ...
        const toastVisible = await page.locator(SELECTORS.notifications.toast).isVisible().catch(() => false);
        expect(toastVisible).toBeTruthy();
      }
    });

    test('puede regenerar una recomendación', async ({ page }) => {
      const regenerateButton = page.locator(SELECTORS.recommendation.regenerateButton).first();
  ...
  test.describe('Navegación desde recomendación', () => {
    test('puede navegar al perfil desde la pantalla de recomendación', async ({ page }) => {
      await page.click(SELECTORS.bottomNav.profile);
      
      await expect(page).toHaveURL(/.*profile.*/);
      await expect(page.locator(SELECTORS.profile.userName).first()).toBeVisible();
    });

    test('puede navegar a la despensa desde la pantalla de recomendación', async ({ page }) => {
      await page.click(SELECTORS.bottomNav.pantry);
      
      await expect(page).toHaveURL(/.*pantry.*/);
    });

    test('puede navegar al plan desde la pantalla de recomendación', async ({ page }) => {
