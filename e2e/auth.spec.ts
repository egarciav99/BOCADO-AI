...
test.describe("Autenticación", () => {
  ...
  test.describe("Registro", () => {
    test("usuario puede registrarse completando el formulario de 3 pasos", async ({
      page,
    }) => {
      ...
      const cityInput = page.locator(
        'input[placeholder*="ciudad" i], input[name="city"]',
      );
      ...
      await page.waitForSelector("text=Objetivo nutricional", {
        timeout: 10000,
      });
      ...
      if (step3Title > 0) {
        ...
        const finalizeBtn = page
          .locator(
            'button:has-text("Crear cuenta"), button:has-text("Finalizar"), button:has-text("Siguiente"), button[type="submit"]',
          )
          .first();
      ...
      await expect(page).toHaveURL(/.*(recommendation|home|app).*/, {
        timeout: 15000,
      });
    ...
    test("muestra error cuando las contraseñas no coinciden", async ({
      ...
      const errorVisible = await page
        .locator(SELECTORS.register.errorMessage)
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Login", () => {
    ...
    test("navegación entre login y registro funciona", async ({ page }) => {
      ...
      await page.waitForSelector(SELECTORS.login.registerLink, {
        state: "visible",
      });
