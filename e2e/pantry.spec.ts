...
test.describe('Despensa (Pantry)', () => {
  ...
  test.describe('Visualización de la despensa', () => {
    test('muestra la lista de la despensa', async ({ page }) => {
      ...
      if (await pantryList.isVisible().catch(() => false)) {
        ...
        const emptyMessage = await page.locator('text=despensa está vacía, No hay productos, Añade tu primer').isVisible().catch(() => false);
  ...
  test.describe('Gestión de items', () => {
    test('puede añadir un nuevo item a la despensa', async ({ page }) => {
      ...
      // Esperar formulario
      await page.waitForSelector(SELECTORS.pantry.itemNameInput, { state: 'visible' });
      ...
      const itemVisible = await page.locator('text=Pollo').isVisible().catch(() => false);
    ...
    test('puede eliminar un item de la despensa', async ({ page }) => {
      ...
      const deleteButton = page.locator(SELECTORS.pantry.deleteItemButton).first();
  ...
  test.describe('Filtrado y búsqueda', () => {
    test('puede filtrar items por categoría', async ({ page }) => {
      ...
      const categoryFilter = page.locator('select[name="categoryFilter"], [data-testid="category-filter"]');
