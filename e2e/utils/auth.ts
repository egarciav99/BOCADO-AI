...
export async function register(page: Page, user: TestUser): Promise<void> {
  ...
  try {
    ...
    await page.screenshot({
      path: "test-failure-start-button.png",
      fullPage: true,
    });
    const html = await page.content();
    console.error("Start button not found. HTML content:", html);
    throw new Error(
      `Start button not found. Screenshot saved. Error: ${error}`,
    );
  }
  ...
  try {
    ...
    const permissionsTitle = page.locator(
      'h1:has-text("Protegemos"), h1:has-text("protect")',
    );

    // Si existe la pantalla de permisos, manejarla
    const isVisible = await permissionsTitle
    ...
    if (isVisible) {
      ...
      const consentArea = page
        .locator("text=Entiendo y acepto, text=agree")
        .first();
  ...
  await page.waitForSelector(SELECTORS.register.firstNameInput, {
    state: "visible",
  });
  ...
  if (user.activityLevel) {
    await page.selectOption(
      SELECTORS.register.activityLevelSelect,
      user.activityLevel,
    );
  }
  ...
  await page.waitForSelector(SELECTORS.recommendation.mealTypeBreakfast, {
    state: "visible",
    timeout: 10000,
  });
}
...
export async function registerBasic(page: Page, user: TestUser): Promise<void> {
  ...
  try {
    ...
    await page.screenshot({
      path: "test-failure-start-button-basic.png",
      fullPage: true,
    });
    const html = await page.content();
    require("fs").writeFileSync("test-failure-start-button-basic.html", html);
    throw new Error(
      `Start button not found in registerBasic. Screenshot and HTML saved. Error: ${error}`,
    );
  }
