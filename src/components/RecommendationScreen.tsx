...
const RecommendationScreen: React.FC<RecommendationScreenProps> = ({
  ...
  const handleGenerateRecommendation = async () => {
    ...
    const isHomeSelectionComplete =
      recommendationType === "En casa" && selectedMeal;
    const isAwaySelectionComplete =
      recommendationType === "Fuera" &&
      selectedCravings.length > 0 &&
      selectedBudget;

    if (!profile || (!isHomeSelectionComplete && !isAwaySelectionComplete))
      return;
    ...
    const cravingsList =
      recommendationType === "Fuera" && selectedCravings.length > 0
  ...
  return (
    <div
      className="flex-1 flex flex-col px-4 py-4 min-h-0 overflow-y-auto"
      role="main"
    >
      {/* Header */}
      ...
      {error && (
        <div
          className="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm animate-fade-in"
          role="alert"
          aria-live="assertive"
        >
        ...
        <div className="flex-1 animate-fade-in">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-lg font-bold text-bocado-green bg-white px-3 py-1 rounded-full">
                        {cookingTime >= 65 ? "60+" : cookingTime} min
                      </span>
                    </div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (
                            ...
                            trackEvent("recommendation_time_adjusted", {
                              time: newValue,
                            });
                          }
                        }}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <span className="text-xs text-bocado-dark-gray">
                      {userPosition ? (
                        <span className="text-bocado-green font-medium">
                          {t("recommendation.locationActive")}
                        </span>
                      ) : locationPermission === "denied" ? (
                        <span className="text-red-600">
                          {t("recommendation.locationDenied")}
                        </span>
                      ) : locationLoading ? (
                        <span className="text-bocado-gray">
                          {t("recommendation.gettingLocation")}
                        </span>
                      ) : (
                        <span className="text-bocado-gray">
                          {t("recommendation.useCurrentLocation")}
                        </span>
                      )}
                    </span>
                  </div>
                  {!userPosition && locationPermission !== "denied" && (
                    ...
                    >
                      {locationLoading
                        ? "..."
                        : t("recommendation.activateLocation")}
                    </button>
                  )}
                </div>
          ...
          <div className="mt-6 transition-all duration-300">
            <button
              ...
              aria-label={t("recommendation.generateAria", {
                default: "Generar recomendación",
              })}
