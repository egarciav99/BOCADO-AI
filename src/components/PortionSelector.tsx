...
const PortionSelector: React.FC<PortionSelectorProps> = ({
  ...
  // ✅ FIX: checkScrollable con debounce para evitar recálculos en cada frame
  ...
  const mainOptions = PORTION_OPTIONS.slice(0, 6);
  const extraOptions = PORTION_OPTIONS.slice(6);
        ...
        >
          {mainOptions.map((option) => (
            <button
              ...
              // ✅ FIX: role y aria-checked para accesibilidad
              role="radio"
              aria-checked={value === option.value}
              aria-label={t("portions.option", { count: option.value })}
              className={`
            ...
            <div className="flex gap-2">
              {extraOptions.map((option) => (
                <button
                  key={option.value}
                  onClick={() => onChange(option.value)}
                  // ✅ FIX: role y aria-checked aquí también
                  role="radio"
                  aria-checked={value === option.value}
                  aria-label={t("portions.option", { count: option.value })}
                  className={`
                  ...
                  `}
                >
                  {/* ✅ FIX: emoji viene de option.icon como el resto */}
