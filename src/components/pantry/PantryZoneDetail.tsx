...
const getFreshnessColor = (status: Freshness) => {
  switch (status) {
    case "fresh":
      return "border-green-400/50 bg-green-50/30";
    case "soon":
      return "border-yellow-400/50 bg-yellow-50/30";
    case "expired":
      return "border-red-500/50 bg-red-50/30";
  }
};
...
export const PantryZoneDetail: React.FC<PantryZoneDetailProps> = ({
  ...
  const translateIngredient = (name: string): string => {
    const ingredientMap: Record<string, string> = {
      ...
      Té: "te",
      Tomate: "tomate",
      "Tomate Frito": "tomateFrito",
      Uvas: "uvas",
      "Verduras Cong.": "verdurasCong",
      Vinagre: "vinagre",
      Vino: "vino",
      Yogur: "yogur",
      Zanahoria: "zanahoria",
  ...
  const zoneItems = useMemo(() => {
    ...
    if (activeCategory !== "Todos") {
      filtered = filtered.filter(
        (i) =>
          i.category.includes(activeCategory.split(" ")[0]) ||
          i.category === "Varios",
      );
    }
    return filtered;
  ...
  const urgentItems = useMemo(
    () =>
      inventory.filter(
        (i) => i.freshness === "expired" || i.freshness === "soon",
      ),
    [inventory],
  );
  ...
  const suggestedItems = useMemo(() => {
    ...
    if (activeCategory === "Todos") {
      const priorityCategories = ZONE_CATEGORIES[zone].filter(
        (c) => c !== "Todos",
      );
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    const newItem: KitchenItem = {
      ...
      zone,
      category:
        activeCategory === "Todos" ? "Varios" : activeCategory.split(" ")[0],
      freshness: "fresh",
      addedAt: Date.now(),
    };
    ...
    <div className="flex-1 flex flex-col bg-white relative">
      <div className="sticky top-0 bg-white z-20 border-b border-bocado-border/50 px-4 py-3">
        <div className="flex items-center gap-2 mb-3">
          ...
          {isSaving && (
            <span className="text-xs text-bocado-gray animate-pulse">
              {t("pantry.saving")}
            </span>
          )}
        </div>
            ...
            <input
              ...
              placeholder={
                activeCategory === "Todos"
                  ...
                  : t("pantry.addInCategory", { category: activeCategory })
              }
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {zoneItems.map((item) => (
              ...
              >
                ...
                <div
                  className={`w-1.5 h-1.5 rounded-full mt-0.5 ${getFreshnessRing(item.freshness)}`}
                />
              </div>
            ))}
          </div>
