...
const NotificationTokensAdmin: React.FC<NotificationTokensAdminProps> = ({
  userUid,
  onBack,
}) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setSettings(
        settingsSnap.exists() ? (settingsSnap.data() as SettingsRecord) : null,
      );
    ...
    <div className="animate-fade-in">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-bocado-gray">
              Tokens y estado de recordatorios
            </p>
          </div>
        </div>
        <button
      ...
      <div className="mb-4 p-4 rounded-xl border border-bocado-border bg-white">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-bold text-bocado-gray uppercase tracking-wide">
            Resumen
          </h3>
          <button
            onClick={loadData}
        ...
        <div className="grid grid-cols-2 gap-3 text-xs text-bocado-dark-gray">
          ...
          <div>
            ...
            <p className="font-semibold">
              {formatTimestamp(settings?.tokenUpdatedAt)}
            </p>
          </div>
          <div>
            <p className="text-bocado-gray">Ultima actividad</p>
            <p className="font-semibold">
              {formatTimestamp(settings?.lastActiveAt)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {tokens.map((token) => (
          <div
            key={token.id}
            className="p-4 rounded-xl border border-bocado-border bg-white"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-2 text-2xs text-bocado-gray space-y-1">
                  ...
                  <p>
                    Actualizado:{" "}
                    {formatTimestamp(token.updatedAt || token.createdAt)}
                  </p>
                </div>
              </div>
