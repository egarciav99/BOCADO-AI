...
const translateActivityFrequency = (
  freq: string,
  t: (key: string) => string,
): string => {
  const map: Record<string, string> = {
...
const Step3: React.FC<FormStepProps> = ({ data, updateData, errors }) => {
  ...
  const handleAddCustomFood = () => {
    ...
    if (
      trimmedInput &&
      !dislikedFoods.find(
        (food) => food.toLowerCase() === trimmedInput.toLowerCase(),
      )
    ) {
          ...
          {Object.keys(filteredCategories).map((category) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  trackEvent("registration_dislike_category_open", {
                    category,
                  }); // ✅ Analítica
              ...
              >
                ...
                <span className="font-bold text-[10px] text-bocado-text leading-tight">
                  {t(`foodCategories.${category}`)}
                </span>
                {dislikedCount > 0 && (
        ...
        <div className="mt-3 space-y-2">
          {customDislikes.length > 0 && (
            <div className="p-3 bg-bocado-background rounded-xl">
              <p className="text-2xs font-bold text-bocado-dark-gray mb-2 uppercase">
                {t("step3.addedManually")}
              </p>
              <div className="flex flex-wrap gap-2">
      ...
      {modalCategory && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-fade-in">
          <div className="bg-white p-4 rounded-2xl shadow-bocado w-full max-w-sm max-h-[80vh] flex flex-col">
            <h3 className="text-base font-bold text-bocado-dark-green mb-3">
              {t(`foodCategories.${modalCategory}`)}
            </h3>
