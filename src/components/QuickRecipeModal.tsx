...
const QuickRecipeModal: React.FC<QuickRecipeModalProps> = ({
  ...
  // ✅ FIX: cancelar request en vuelo si el modal se cierra
  useEffect(() => {
    return () => {
  ...
  // ✅ FIX: limpiar undo timeout al desmontar
  useEffect(() => {
    return () => {
  ...
  const handleInputChange = (value: string) => {
    if (value.length > MAX_INGREDIENT_LENGTH) return;
    setInputValue(value);
    setError(null);

    if (value.trim() && !isValidIngredientInput(value)) {
      setError(t("quickRecipe.invalidIngredient"));
      setSuggestions([]);
      return;
    }
    ...
    const filtered = allIngredients
      .filter((ing) => {
        ...
        return `${name} ${mx} ${es}`.includes(normalized);
      })
      .filter((ing) =>
  ...
  // ✅ FIX: t removido de deps — es estable en la mayoría de implementaciones
  // y si no lo es, el efecto de cambio de locale re-monta el componente de todas formas
  const addIngredient = useCallback(
    (ingredientName: string) => {
      ...
      if (normalized.length > MAX_INGREDIENT_LENGTH) {
        setError(t("quickRecipe.ingredientTooLong"));
        return;
      }
      ...
      if (
        ...
        setError(t("quickRecipe.ingredientDuplicate"));
        return;
      }

      if (selectedIngredients.length >= 15) {
        setError(t("quickRecipe.maxIngredientsReached"));
        return;
      }
  ...
  const removeIngredient = (ingredientName: string) => {
    // ✅ FIX: limpiar timeout anterior via ref antes de crear uno nuevo
    if (undoTimeoutRef.current) {
      clearTimeout(undoTimeoutRef.current);
    }
  ...
  const handleUndoRemove = () => {
    if (undoSnackbar) {
      ...
      setUndoSnackbar(null);
      trackEvent("quick_recipe_ingredient_undo", {
        ingredient: undoSnackbar.ingredient,
      });
    }
  };
  ...
  const handleGenerateRecipe = async () => {
    ...
    if (selectedIngredients.length < 2) {
      setError(t("quickRecipe.minIngredientsError"));
      return;
    }

    if (!user || !profile) {
      setError(t("quickRecipe.userNotFound"));
      return;
    }
    ...
    try {
      ...
      const requestBody = {
        ...
        _id: newDoc.id,
        // ✅ FIX: usar locale del context en vez de hardcodear "es"
        language: locale,
      };
      ...
      timeoutId = setTimeout(() => {
        ...
        setError(t("quickRecipe.connectionError"));
      ...
      if (!response.ok) {
        if (response.status === 429) {
          setError(t("quickRecipe.rateLimited"));
        } else {
          const errorText = await response.text();
          setError(t("quickRecipe.generationError"));
          logger.error("Quick recipe API error:", errorText.substring(0, 100));
        }
        isProcessingRef.current = false;
        setIsGenerating(false);
      ...
      if (err.name === "AbortError") {
        ...
        setError(t("quickRecipe.unknownError"));
      }
      isProcessingRef.current = false;
      setIsGenerating(false);
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full md:max-w-md bg-white dark:bg-gray-800 rounded-t-3xl md:rounded-3xl p-5 space-y-5 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center gap-3">
          ...
          {/* ✅ FIX: ícono X de Lucide + aria-label traducido */}
          <button
            onClick={onClose}
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-bocado-dark-gray dark:text-gray-300 uppercase tracking-wide">
            {t("quickRecipe.ingredientsLabel")}
          </label>
          <div className="relative">
            <input
              ...
              placeholder={t("quickRecipe.ingredientPlaceholder")}
          ...
          <div className="space-y-2">
              ...
              {selectedIngredients.map((ing) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={t("quickRecipe.removeIngredient", { name: ing })}
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
            ...
            <p className="text-xs text-bocado-gray dark:text-gray-400 font-medium">
              {selectedIngredients.length === 1
                ? t("quickRecipe.ingredientCountSingular")
                : t("quickRecipe.ingredientCountPlural", {
        ...
        <div className="bg-bocado-background dark:bg-gray-700 rounded-2xl p-3 space-y-3">
          ...
          >
            <span>
              ⏱️{" "}
              {cookingTime
                ? t("quickRecipe.cookingTimeValue", { time: cookingTime })
                : t("quickRecipe.cookingTimeLabel")}
            </span>
            <span
              className={`text-lg transition-transform ${showTimeSlider ? "rotate-180" : ""}`}
            >
              ▼
            </span>
          </button>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-bocado-gray dark:text-gray-400">
                    {t("quickRecipe.customizeLabel")}
                  </span>
                  <span className="text-sm font-bold text-bocado-green">
                    {cookingTime || 20} min
                  </span>
                </div>
                <input
        ...
        <div className="flex gap-2 pt-2 border-t border-bocado-border dark:border-gray-700">
          ...
          >
            {t("quickRecipe.cancelButton")}
          </button>
          <button
            onClick={handleGenerateRecipe}
            disabled={isGenerating || selectedIngredients.length < 2 || loading}
          ...
          >
            {isGenerating ? (
              <>
                ...
                {t("quickRecipe.generatingButton")}
              </>
            ) : (
              <>⚡ {t("quickRecipe.generateButton")}</>
            )}
          </button>
        </div>
      </div>

      {/* ✅ FIX: tutorial eliminado — ya vive en QuickRecipeButton (el padre) */}
      ...
      {undoSnackbar && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-800 dark:bg-gray-900 text-white px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-slide-up z-50 max-w-sm">
          <span className="text-sm flex-1">
            {undoSnackbar.ingredient} {t("quickRecipe.willBeRemoved")}
          </span>
          <button
