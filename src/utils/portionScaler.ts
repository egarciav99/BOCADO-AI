...
const CATEGORY_KEYWORDS: Record<IngredientCategory, string[]> = {
  protein: [
    'pollo', 'pechuga', 'muslo', 'alita', 'pavo', 'pata', 'carne', 'res', 'cerdo', 'chuleta',
    'pescado', 'pesca', 'salmón', 'atún', 'merluza', 'bacalao', 'filete', 'filete',
    'huevo', 'huevos', 'huevo', 'clara', 'yema',
    'tofu', 'tempeh', 'seitán', 'proteína',
    'lenteja', 'garbanzo', 'frijol', 'poroto', 'alubia', 'haba',
    'jamón', 'tocino', 'chorizo', 'salchicha', 'morcilla', 'embutido',
  ],
  carb: [
    'arroz', 'pasta', 'fideo', 'espagueti', 'macarrón', 'lasaña', 'ñoqui', 'gnocchi',
    'papa', 'patata', 'camote', 'batata', 'yuca', 'ñame', 'plátano macho', 'plátano',
    'pan', 'tortilla', 'arepa', 'tostada', 'pan', 'baguette',
    'avena', 'quinoa', 'cebada', 'trigo', 'maíz', 'harina',
  ],
  vegetable: [
    'tomate', 'jitomate', 'lechuga', 'espinaca', 'acelga', 'kale', 'col', 'repollo', 'berza',
    'zanahoria', 'pepino', 'calabacín', 'calabaza', 'berenjena', 'pimiento', 'chile', 'ají',
    'brócoli', 'coliflor', 'espárrago', 'judía', 'ejote', 'chicharo', 'guisante',
    'champiñón', 'hongo', 'seta', 'palmito', 'alcachofa', 'apio', 'rábano', 'nabo',
    'aguacate', 'palta', 'remolacha', 'betabel', 'puerro', 'puerro',
  ],
  aromatic: [
    'cebolla', 'ajo', 'chalote', 'cebollín', 'cebollita', 'puerro', 'apio',
    'perejil', 'cilantro', 'albahaca', 'menta', 'romero', 'tomillo', 'orégano', 'laurel',
    'jengibre', 'cúrcuma', 'raíz', 'hierba', 'hoja',
  ],
  spice: [
    'sal', 'pimienta', 'comino', 'canela', 'clavo', 'nuez moscada', 'pimentón', 'paprika',
    'cayena', 'chil', 'picante', 'orégano seco', 'tomillo seco', 'romero seco',
    'azafrán', 'cárdamo', 'cilantro seco', 'comino', 'cúrcuma', 'curry', 'masala',
    'condimento', 'especia', 'mezcla', 'adobo', 'sazonador', 'caldo', 'consomé',
    'vainilla', 'extracto', 'esencia',
  ],
  liquid: [
    'agua', 'caldo', 'consomé', 'leche', 'crema', 'nata', 'yogur', 'kefir',
    'aceite', 'vinagre', 'jugo', 'zumo', 'limón', 'lime', 'naranja',
    'vino', 'cerveza', 'licor', 'alcohol', 'salsa de soya', 'salsa soya', 'salsa inglesa',
    'aceite de oliva', 'aceite de coco', 'mantequilla derretida',
  ],
  dairy: [
    'queso', 'mozzarella', 'parmesano', 'cheddar', 'feta', 'crema', 'nata',
    'mantequilla', 'margarina', 'yogur', 'yogurt', 'leche', 'leche evaporada', 'leche condensada',
  ],
  other: [],
};
...
const MINIMUMS: Record<string, number> = {
  'huevo': 1, 'huevos': 1,
  'cebolla': 0.5, 'cebollas': 0.5,
  'diente de ajo': 1, 'dientes de ajo': 1,
  'cabza de ajo': 0.5, 'cabezas de ajo': 0.5,
  'limón': 0.5, 'limones': 0.5,
  'lima': 0.5, 'limas': 0.5,
  'tomate': 0.5, 'tomates': 0.5, 'jitomate': 0.5, 'jitomates': 0.5,
};
 ...
 */
function extractNumber(text: string): { value: number; hasNumber: boolean; rest: string } {
  ...
  for (const [char, value] of Object.entries(fractions)) {
    if (trimmed.startsWith(char)) {
      return { value, hasNumber: true, rest: trimmed.slice(char.length).trim() };
    }
  }
...
function formatNumber(num: number, category: IngredientCategory): string {
  ...
  const min = detectMinimum('placeholder') || SCALING_CONFIG[category].minIncrement;
  if (num > 0 && num < min) {
    num = min;
  }
 ...
 */
export function calculateMultiplier(baseServings: number, targetServings: number): number {
